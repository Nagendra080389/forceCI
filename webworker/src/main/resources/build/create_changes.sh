...
then
        ...
        while read CFILE
            do
                    case "$CFILE"
                    in
                        src/aura*) TYPENAME="AuraDefinitionBundle";;
                        src/lwc*) TYPENAME="LightningComponentBundle";;
                        *) TYPENAME="UNKNOWN";;
                    esac
                    if [ "$TYPENAME" == "AuraDefinitionBundle" ]
                        then
                                ...
                                then
                                    ...
                                    sed "s|$CFILENAME|$replace|" destructive.txt >> temp_destructive.txt
                                    mv -- temp_destructive.txt destructive.txt
                                else
                                    continue
                                fi
                        fi
                    if [ "$TYPENAME" == "LightningComponentBundle" ]
                        then
                                ...
                                then
                                    CFILENAME="$CFILE"
                                    ARRAY+=($(basename -- "$(dirname -- "$CFILE")"))
                                    echo 'Entered If -> ' $ARRAY
                                    replace="src/lwc/"$(basename -- "$(dirname -- "$CFILE")")
                                    sed "s|$CFILENAME|$replace|" destructive.txt >> temp_destructive.txt
                                    mv -- temp_destructive.txt destructive.txt
                                else
                                    continue
